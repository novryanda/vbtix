...
export default function EventDetailPage() {
  ...
  // Fungsi untuk menangani pembelian tiket
  const handleBuyTicket = () => {
    // Cek apakah ada tiket yang dipilih (jumlahnya > 0)
    const hasSelectedTickets = Object.values(ticketCounts).some(
      (count) => count > 0,
    );

    if (hasSelectedTickets) {
      // Buat query string dari ticketCounts untuk dikirim ke halaman order
      const queryParams = new URLSearchParams();

      // Tambahkan data ticketCounts ke query params
      Object.entries(ticketCounts).forEach(([ticketId, count]) => {
        if (count > 0) {
          queryParams.append(`ticket_${ticketId}`, count.toString());
        }
      });

      // Navigate to the order form page dengan query params
