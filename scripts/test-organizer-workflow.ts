...
async function testOrganizerWorkflow() {
  try {
    ...
    users.forEach((user, index) => {
      console.log(`  ${index + 1}. ${user.email} - Role: ${user.role} - Has Organizer: ${user.organizer ? '✅' : '❌'}`);
    });
    ...
    if (organizerUserWithoutRecord) {
      ...
      if (buyerUser) {
        console.log(`   Found BUYER user that could be changed to ORGANIZER: ${buyerUser.email}`);
      }
    }
    ...
    pendingVerifications.forEach((request, index) => {
      console.log(`  ${index + 1}. ${request.user?.email} - Submitted: ${request.submittedAt.toLocaleDateString()}`);
    });
    ...
    console.log(`Found ${organizerVerifications.length} pending organizer verifications:`);
    organizerVerifications.forEach((verification, index) => {
      console.log(`  ${index + 1}. ${verification.organizer.user.email} - Submitted: ${verification.submittedAt?.toLocaleDateString()}`);
    });

    // Step 5: Summary
    console.log("\n📊 Workflow Summary:");
    console.log(`  • Total Users: ${users.length}`);
    console.log(`  • Users with ORGANIZER role: ${users.filter(u => u.role === "ORGANIZER").length}`);
    console.log(`  • Users with Organizer records: ${users.filter(u => u.organizer).length}`);
    console.log(`  • Pending user verification requests: ${pendingVerifications.length}`);
    console.log(`  • Pending organizer verifications: ${organizerVerifications.length}`);

    // Step 6: Workflow status
    console.log("\n🔄 Workflow Status:");
    
    if (pendingVerifications.length > 0) {
      console.log("  ✅ Ready for admin approval - User verification requests pending");
      console.log("  📝 Next step: Admin should review and approve in /admin/organizers");
    } else if (organizerVerifications.length > 0) {
      console.log("  ✅ Ready for admin approval - Organizer verifications pending");
      console.log("  📝 Next step: Admin should review and approve in /admin/organizers");
    } else {
      console.log("  ℹ️  No pending verifications");
      console.log("  📝 Next step: Create a user verification request in /organizer/[id]/verification");
    }
    ...
    console.log("  1. Go to /admin/users - Change a BUYER user to ORGANIZER role");
    console.log("  2. Go to /organizer/[userId]/verification - Submit verification as that user");
    console.log("  3. Go to /admin/organizers - Review and approve the verification");
    console.log("  4. Verify organizer record is created and user can access organizer features");
